...
public class MainActivity extends AppCompatPreferenceActivity implements DialogInterface.OnClickListener, CompoundButton.OnCheckedChangeListener, WifiApEnabler.OnStateChangeListener, Preference.OnPreferenceChangeListener {
...
//        mTracker = AnalyticsApplication.getDefaultTracker(this);
    ...
    private void initWifiTethering() {
        ...
        if (mWifiConfig == null) {
            ...
            final String summary = String.format(getString(R.string.wifi_tether_configure_subtext), s, mSecurityType[WifiApDialog.OPEN_INDEX]);
            mCreateNetwork.setSummary(summary);
        } else {
            int index = WifiApDialog.getSecurityTypeIndex(mWifiConfig);
            final String summary = String.format(getString(R.string.wifi_tether_configure_subtext), mWifiConfig.SSID, mSecurityType[index]);
            mCreateNetwork.setSummary(summary);
        }
    }
        ...
        if (mApEnabled) {
            ...
            if ((mLastClientList != null) && (mActivity != null)) {
                ...
                for (ClientsList.ClientScanResult client : mLastClientList) {
                    if (!client.isReachable) {
                        Preference preference = new Preference(mActivity);
                        preference.setTitle(client.hwAddr + "   " + getString(R.string.disconnected));
                        preference.setSummary(client.ipAddr + "   " + client.vendor);
                        //preference.setEnabled(client.isReachable);
                        mClientsCategory.addPreference(preference);
                    }
                ...
                try {
                    ...
                    for (String client : blockedList) {
                        Preference preference = new Preference(mActivity);
                        preference.setTitle(client + "   " + getString(R.string.blocked));
                        preference.setSummary("___.___.___.___   " + ClientsList.getVendor(client, MainActivity.this));
                        mClientsCategory.addPreference(preference);
                    }
...
//            mTracker.send(new HitBuilders.EventBuilder().setCategory("Action").setAction("AP Settings Dialog").build());
        } else {
            if (mac.matches("..:..:..:..:..:..")) {
//                mTracker.send(new HitBuilders.EventBuilder().setCategory("Action").setAction("Device clicked").build());
...
//                        mTracker.send(new HitBuilders.EventBuilder().setCategory("Action").setAction("Device blocked").build());
                    }
                });
...
//            mTracker.send(new HitBuilders.EventBuilder().setCategory("Action").setAction("Dialog BUTTON_POSITIVE").build());
            ...
            if (mWifiConfig != null) {
                ...
                int index = WifiApDialog.getSecurityTypeIndex(mWifiConfig);
                String summary = String.format(getString(R.string.wifi_tether_configure_subtext), mWifiConfig.SSID, mSecurityType[index]);
                mCreateNetwork.setSummary(summary);
            }
        }
    }
...
//        mTracker.send(new HitBuilders.EventBuilder().setCategory("Action").setAction("onCheckedChanged " + isChecked).build());
        if (isChecked) {
            startProvisioningIfNecessary();
        } else {
            if (!mIsRestarting) {
    ...
    private final class ClientScanHandler extends Handler {
        ...
        @Override
        public void handleMessage(Message msg) {
            ArrayList<ClientsList.ClientScanResult> clients = ClientsList.get(mActivity);
